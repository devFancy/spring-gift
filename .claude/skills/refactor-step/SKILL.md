...
spring-gift 리팩터링 한 조각을 받아 단계별로 진행하는 워크플로우. 과제5 4번 원칙(단일 목적 커밋)과 5번 원칙(한 번에 한 조각)을 실행 흐름으로 갖는다. 본 과제는 리팩터링 전용이라 새 작동 도입은 본 흐름에 포함하지 않는다.
...
입력된 변경이 단일 목적인지(예: 패키지 재배치 한 가지, 트랜잭션 경계 명시 한 가지, 도메인 정책 회수 한 가지) 사용자와 함께 확인한다. 두 가지 이상 섞여 있으면 "한 가지만" 으로 좁힌다. 본 과제는 리팩터링 전용이라 새 작동 도입이 포함되어 있다면 즉시 멈추고 사용자에게 보고한다.
...
코드 수정/추가 시: `references/code-style.md` 를 먼저 읽는다. 패키지 배치, 트랜잭션 경계, 검증 분리, DTO 분리 원칙을 따라 변경을 적용한다.

### 4단계: 검증 (작동 보존 증거)

테스트 작성/실행 시: `references/testing-guide.md` 를 먼저 읽는다. 기존 테스트 전체 통과로 작동 보존을 증명한다. 기존 테스트가 없는 영역이면 변경 전에 작동 보존 테스트를 별도 `test:` 커밋으로 먼저 추가한다.

API 추가/변경 시: `references/api-convention.md` 를 먼저 읽는다. ApiResponse 포맷, 검증 분리, ErrorType 사용을 일관되게 적용한다.
