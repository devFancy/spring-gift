...
- 모든 API 응답은 `ApiResponse<T>` 객체로 감싼다. 구조는 `.claude/skills/refactor-step/references/api-convention.md` 참고.
...
- 기본은 주석 없음. WHY 가 비자명할 때만 단다. 상세 규칙은 `.claude/rules/formatting.md` 참고.
