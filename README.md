...
- [x] 카테고리 - 카테고리 관리 흐름 정리, ControllerDocsTest 로 외부 계약 검증 (단순 CRUD 라 ServiceTest 미작성)
- [x] 상품 옵션 - 상품 옵션 관리 흐름 정리, ControllerDocsTest 와 ServiceTest 함께 작성
- [x] 주문 - 주문 생성 흐름 정리, ControllerDocsTest 와 ServiceTest 함께 작성, 카카오 메시지 클라이언트 분리
...
도메인-영속성 분리의 실질적 이점을 검증하지 않고 도입했습니다.

도메인-영속성 분리(`domain/` 리포지토리 인터페이스 -> `storage/` JPA 엔티티 + 리포지토리 구현체)를 적용하면서, 현재 맥락에서 이 패턴이 실제로 필요한지 먼저 확인하지 않았습니다. 
도메인 하나당 리포지토리 인터페이스, JPA 엔티티, 리포지토리 구현체 세 파일이 추가됩니다. JPA 를 교체할 계획이 없는 지금 규모에서 이 추가 비용이 정당화되는지 먼저 따졌어야 했습니다. 
패턴이 이론적으로 올바르더라도 현재 팀과 규모에서 감당할 복잡도인지를 기준으로 판단했어야 한다는 점이 아쉬웠습니다.