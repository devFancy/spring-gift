...
통합 테스트 중심 전략을 채택한다. 도메인마다 두 종류의 테스트를 짝지어 둔다.

- `{Domain}ControllerDocsTest` 서비스 Mock 기반 독립 테스트. API 계약(URL, 인증, status code, 응답 필드) 문서화.
- `{Domain}ServiceTest` ControllerDocsTest 가 닿지 않는 시나리오 (도메인 정책 분기, idempotent 보장, 운영자 흐름) 검증. 실제 DB 기반.
...
- 단순 CRUD 도메인 (예: 카테고리) 처럼 ServiceTest 가 ControllerDocsTest 와 시나리오가 겹치면 ServiceTest 를 통째 들어내는 판단도 가능해진다.
- 컨텍스트는 동일한 설정을 공유하는 통합 테스트 전체에서 한 번만 부팅되고 재사용된다. 부팅 비용 자체는 크지 않지만, 전체 컨텍스트를 올려야 하므로 순수 단위 테스트보다 실행 시간이 길다.
