...
| 카카오 액세스 토큰 | kakaoAccessToken | 카카오 메시지 전송에 사용하는 OAuth 액세스 토큰. 카카오 소셜 로그인 회원에게만 존재. null 허용 |
...
| 인증 주체 | Member | 인증된 회원 객체. API 요청 시 인증 헤더를 통해 식별된다 |
...
>
> 본 프로젝트에서는 도메인 객체(`domain/`)와 JPA 엔티티(`storage/`)를 분리합니다. 모델링은 순수 도메인 객체를 중심으로 진행하며, 영속성 세부 사항은 `storage/` 계층에서 처리합니다.
...
- 같은 상품 내에서 옵션명은 중복될 수 없다.
- 상품의 마지막 옵션은 삭제할 수 없다.
